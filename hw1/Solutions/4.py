...
def plot_regression_alpha(pred_dict:dict, y:np.ndarray, X:np.ndarray, file_path:str):
    ...
    for i in range(len(pred_dict.keys())):
        ...
        axs[i//3, i%3].plot(X, y)
        axs[i//3, i%3].plot(X, y_pred)
        alpha_ = np.around(alpha/len(y_pred), decimals=5)
...
if __name__ == '__main__':
    ...
    P = [101, 117, 149, 167, 185, 201]
    predictions_ridgereg_p = prediction_over_p(P, alpha=0.0001 * len(data_dict["y"]), X=data_dict["X"][:-1], y=data_dict["y"][1:])
    plot_regression(predictions_ridgereg_p, data_dict["y"][1:], data_dict["X"][:-1], ridge=True, file_path="predictions_ridge_p_search.png")

    alphas = [1* len(data_dict["y"]), 0.1* len(data_dict["y"]), 0.01* len(data_dict["y"]), \
              0.001* len(data_dict["y"]), 0.0001* len(data_dict["y"]), 0.00001* len(data_dict["y"])] 

    predictions_ridgereg_alpha = prediction_over_alpha(P = 167, alphas=alphas, X=data_dict["X"][:-1], y=data_dict["y"][1:])
    plot_regression_alpha(predictions_ridgereg_alpha, data_dict["y"][1:], data_dict["X"][:-1], file_path="predictions_ridge_alpha_search.png")
    ...
    P=6
    ridge = RidgeRegression(num_dims=P, alpha=0.000000001*len(data_dict["y"]))
    ...
    pred = ridge.predict(X_train_test)

    fig, ax = plt.subplots(figsize=(15, 15))
    ax.plot(X[:, 1], data_dict["y"], linewidth=7, label="True")
    ax.plot(X_train_test[:, 1], pred, linewidth=10, linestyle="dashed", label = "Pred")
    ax.set_xlabel("Weeks after first reading", fontsize=30)
    ax.set_ylabel(f"Dew Point Temp (C)", fontsize=30)
    #ax.set_title("", fontsize=40)
    ax.tick_params(axis='both', which='major', labelsize=30)
